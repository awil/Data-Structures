...
public class MinHeap {
	 ...
	 * 				right position. Smallest value to the top
    ...
    public void trickleUp(int index) {
    	...
    	while(index > 0 && heap.get(parent).getKey().compareTo(bottom.getKey()) >= 0) {
    		heap.set(index, heap.get(parent)); // move it down
	        index = parent;
	        parent = (parent-1)/2;
	    }
	...
	/*
	 * Description:	This method finds the larger children and moves them down
	 * Precondition: Index param passed in
    ...
    public void trickleDown(int index) {
	    ...
	    while(index < (size/2)) {
	        ...
	        if(top.getKey() <= heap.get(largerChild).getKey()) {
	        	break;
	        }
	        // move child up
